...
GRANT SELECT , 
	...
	ON $dbname.* TO 
	'$dbuser'@'192.168.0.%' IDENTIFIED BY '$dbpassword';
...
-- DROP INDEX #IF_INDEX_EXISTS() username #ON("users");
...
-- DROP INDEX #IF_INDEX_EXISTS() roleid #ON("roles");
...
-- DROP INDEX #IF_INDEX_EXISTS() userrole #ON("userroles");
...
#SQLECHO("---- all users (2 were deleted) ----")	
select * from users;

#SQLECHO("---- user with its role (only admin...) ----")	
select users.username,roles.role from users,roles,userroles 
	where users.username=userroles.username 
	and userroles.roleid=roles.id;
